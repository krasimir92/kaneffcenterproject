...
<script type='text/javascript' src='http://ajax.googleapis.com/ajax/libs/jquery/1.7.0/jquery.min.js?ver=1.7.0'></script>
	...
	<div class="header">
					...
					<button class="buttonbg" value="Refresh Page" onclick="window.location.href='delete.php'"><a class="effect-underline">БГ</a></button>
					<button class="buttonen" value="Refresh Page" onclick="window.location.href='delete_en.php'"><a class="effect-underline">EN</a></button>
...
<form method="post" action="programa_en.php">
    <h3>Delete performance</h3>
    <select name="id_predst">
      <?php
      $sql = mysqli_query($conn, "SELECT id_predst,ime_predst FROM predstavleniq_en");
      while ($row = mysqli_fetch_assoc($sql)){?>
        <option value="<?php echo $row['id_predst']; ?>"><?php echo $row['ime_predst']; ?></option>
      <?php }?>
    </select>
<input type="submit" name="delete" value="Delete performance">
</form>
